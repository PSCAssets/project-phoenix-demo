# Project Phoenix Demo — Meeting Update Script v3
...
## CHANGE 4 — Provider Chart: Hide SLA Display Unless Alert is Triggered

**File:** `templates/provider/chart.html`

The chart header currently shows an SLA time remaining box (e.g., "1h 24m" in amber), and the stage bar shows an SLA badge. These elements should **not appear at all** unless an SLA threshold configured in the admin portal has actually been breached or is approaching a warning state.

Showing SLA information when everything is fine is unnecessary — providers should only see SLA indicators when action is required.

### Rule:

- **If SLA is in a warning or alert state** (20% or less of SLA time remaining, or SLA breached): show the SLA indicator in the chart header and stage bar
  - Amber `#D97706` / background `#FEF3C7`: 20–50% remaining — label `"⚠ SLA: Xh Ym remaining"`
  - Red `#DC2626` / background `#FEE2E2`: less than 20% remaining — label `"🚨 SLA: Xh Ym — Action Required"`
- **If SLA is healthy** (more than 50% of SLA time remaining): **hide the SLA element entirely** — do not show it at all

### For the demo:

Marcus Johnson's consult has ample SLA time remaining (48-hour SLA, only a few hours elapsed). This is a healthy state. **Remove the SLA box from the chart header entirely** and **remove the SLA badge from the stage bar entirely**. No SLA element should appear for this patient in the demo.

Add a JS comment where the SLA logic lives: `// SLA display: only show when warning/alert threshold is triggered. Hidden when SLA is healthy (>50% remaining).`
...
Below the status card, a thin progress bar showing the patient's SLA window:
- Label left: `"SLA: 4 hrs"` — Label right: `"2h 24m remaining"`
- Bar: 60% filled, color `#F59E0B` (amber — patient is in the later portion of their SLA window)
- Bar background: `#FEF3C7`
- Height: 6px, border-radius 3px

Note: This amber bar is intentional for the patient portal. It communicates to the patient where they are in the SLA window. The provider-side chart (Change 4) hides SLA entirely because the provider's configured SLA threshold has not been triggered.
